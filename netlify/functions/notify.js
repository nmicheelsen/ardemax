...
exports.handler = async function(event) {
  ...
  try {
    const { disciplina, area, descripcion, supervisor } = JSON.parse(event.body);

    // Get subscriptions from Sheets
    const res = await fetch(`${SHEETS_URL}?accion=leer_suscripciones&disciplina=${disciplina}`);
    const data = await res.json();
    const suscripciones = data.suscripciones || [];

    const payload = {
      title: `Bloqueo en ${area || 'obra'}`,
      body: `${supervisor}: ${descripcion}`,
      icon: '/icon-192.png'
    };

    const resultados = await Promise.allSettled(
      suscripciones.map(s => sendPushNotification(s.subscription, payload))
    );
